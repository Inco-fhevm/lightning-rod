...
contract TestSimpleConfidentialToken is IncoTest {
    ...
    function setUp() public override {
        ...
        token.transfer(alice, fakePrepareEuint256Ciphertext(10 * GWEI, alice, address(token)));
    }

    function testTransfer() public {
        vm.prank(alice);
        token.transfer(bob, fakePrepareEuint256Ciphertext(1 * GWEI, bob, address(token)));
